...
def positional_encoding(tensor, L=6):
    """
    Mapping des coordonnées 3D vers un espace de haute fréquence.
    Aide le MLP à surmonter le lissage spectral (Spectral Bias).
    """
    encoded_list = [tensor]
    for i in range(L):
        for fn in [tf.sin, tf.cos]:
            encoded_list.append(fn(2.0 ** i * math.pi * tensor))
    return tf.concat(encoded_list, axis=-1)

def local_decoder(feature_f, input_points_3d_f):
    with tf.variable_scope('dis', reuse=tf.AUTO_REUSE):
        
        # --- ETAPE 1 : On transforme les points 3D en hautes fréquences ---
        encoded_points = positional_encoding(input_points_3d_f, L=6)
        
        feature_f = tf.nn.relu(tf.layers.dense(feature_f, 128))
        
        # --- ETAPE 2 : Le premier layer utilise maintenant 'encoded_points' au lieu de 'input_points_3d_f' ---
        net = tf.nn.relu(tf.layers.dense(encoded_points, 512))
        ...
        # --- ETAPE 3 : On calcule le gradient (garde ta logique originale avec safe_norm) ---
        grad = tf.gradients(ys=sdf, xs=input_points_3d_f) 
        normal_p_lenght = tf.expand_dims(safe_norm(grad[0], axis=-1), -1)
        grad_norm = grad[0] / (normal_p_lenght + 1e-12)
        
        g_points = input_points_3d_f - sdf * grad_norm
